...
async def init_db():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if count[0] == 0:
            await db.execute(
                "INSERT INTO settings (id, passport_text, logo_path) VALUES (1, '', '')"
            )
        await db.commit()
...
async def get_settings() -> dict:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cursor = await db.execute(
            "SELECT passport_text, logo_path, updated_at FROM settings WHERE id = 1"
        )
        ...
        return {"passport_text": "", "logo_path": "", "updated_at": None}
